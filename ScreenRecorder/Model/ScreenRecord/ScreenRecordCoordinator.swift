...
@objc class ScreenRecordCoordinator: NSObject
{
    ...
    func startRecording(withFileName fileName: String, recordingHandler: @escaping (URL?) -> Void,onCompletion: @escaping (URL?)->Void)
    {
        if (self.showOverlay!) {
            self.viewOverlay.show()
        }
        screenRecorder.startRecording(withFileName: fileName) { (url) in
            recordingHandler(url)
