...
public protocol TrimmerViewDelegateMusic: AnyObject {
...
// range
@IBDesignable public class TrimmerViewMusic: AVAssetTimeSelectorMusic {
    ...
    @IBInspectable public var maskColor: UIColor = UIColor.black {
        didSet {
    ...
    public weak var delegate: TrimmerViewDelegateMusic?

    // MARK: Subviews
    private let trimView = UIView()
    private let leftHandleView = HandlerViewMusic()
    private let rightHandleView = HandlerViewMusic()
    ...
    private func setupTrimmerView() {
        ...
        trimView.layer.cornerRadius = 4.0
    ...
    private func setupHandleView() {
        ...
        //UIImage -
        let leftImage = UIImage(named: "leftAnchor")
        let leftImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: 3, height: 8))
        leftImageView.image = leftImage
        leftImageView.tintColor = .black
        //============================================
        
        //UIImage -
        let rightImage = UIImage(named: "rightAnchor")
        let rightImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: 3, height: 8))
...
//            leftImageView.centerXAnchor.constraint(equalTo: leftHandleView.centerXAnchor),
//            leftImageView.centerYAnchor.constraint(equalTo: leftHandleView.centerYAnchor),
//        ])
        let aspectR = leftImage!.size.width / leftImage!.size.height
        NSLayoutConstraint.activate([
                leftImageView.centerXAnchor.constraint(equalTo: leftHandleView.centerXAnchor),
                leftImageView.centerYAnchor.constraint(equalTo: leftHandleView.centerYAnchor),
                leftImageView.leadingAnchor.constraint(greaterThanOrEqualTo: leftHandleView.leadingAnchor),
                leftImageView.trailingAnchor.constraint(lessThanOrEqualTo: leftHandleView.trailingAnchor),
                leftImageView.heightAnchor.constraint(equalTo: leftImageView.widthAnchor, multiplier: 1/aspectR)
        ])
...
//        rightHandleView.addSubview(rightHandleKnob)
        ...
        let aspect = rightImage!.size.width / rightImage!.size.height
        NSLayoutConstraint.activate([
                rightImageView.centerXAnchor.constraint(equalTo: rightHandleView.centerXAnchor),
                rightImageView.centerYAnchor.constraint(equalTo: rightHandleView.centerYAnchor),
                rightImageView.leadingAnchor.constraint(greaterThanOrEqualTo: rightHandleView.leadingAnchor),
                rightImageView.trailingAnchor.constraint(lessThanOrEqualTo: rightHandleView.trailingAnchor),
                rightImageView.heightAnchor.constraint(equalTo: rightImageView.widthAnchor, multiplier: 1/aspect)
        ])
...
//        var topView = UIView(frame: CGRect(x: 0, y: 0, width: 50, height: 10))
//        topView.backgroundColor = .blue

        positionBar.frame = CGRect(x: 0, y: 0, width: 2, height: frame.height + 10)
        positionBar.backgroundColor = positionBarColor
        ...
        //For Hide Defualt Position Bar
        positionBar.alpha = 0
        
//        addSubview(topView)
//        topView.translatesAutoresizingMaskIntoConstraints = false
//        NSLayoutConstraint.activate([
//            topView.topAnchor.constraint(equalTo: positionBar.topAnchor),
//            topView.leadingAnchor.constraint(equalTo: positionBar.leadingAnchor),
//            topView.trailingAnchor.constraint(equalTo: positionBar.trailingAnchor),
...
//        ])
    ...
    private func setupGestures() {
        ...
        let leftPanGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(TrimmerViewMusic.handlePanGesture))
        leftHandleView.addGestureRecognizer(leftPanGestureRecognizer)
        let rightPanGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(TrimmerViewMusic.handlePanGesture))
    ...
    public func seek(to time: CMTime) {
        if let newPosition = getPosition(from: time) {
            ...
            //MARK: - Modification for Move Position Time View
            let position = positionBar.frame.origin.x
            delegate?.positionBarPosition(position - 13, time)
        }
    }
