...
protocol QualitySliderValueDelegate {
    func getQualityValue(_ quality: Double)
}

protocol FrameSliderValueDelegate {
    func getFrameRateValue(_ frame: Double)
}
...
final class Slider: UIControl {
    ...
    var qualityDelegate: QualitySliderValueDelegate?
    var frameDelegate: FrameSliderValueDelegate?
    var speedDelegate: SliderValueDelegate?
    
    // MARK: Constants

    enum Constants {
        static let height: CGFloat = 2
        static let trackerInnerCircleHeight: CGFloat = 12.0
        static let trackerOutterCircleHeight: CGFloat = 0.0
        static let trackerHorizontalMargin: CGFloat = 1.0
        static let indicatorHeight: CGFloat = 0.0

        static let backgroundColor: CGColor = UIColor(hex: "#151517").withAlphaComponent(0.04).cgColor
        static let trackOutterCircleBackgroundColor = #colorLiteral(red: 0.9999960065, green: 1, blue: 1, alpha: 1).cgColor
        static let trackerInnerCircleBackgroundColor =  UIColor(named: "newBrandColor")?.cgColor
        static let indicatorsBackgroundColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1).cgColor
    ...
    public override func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        speedDelegate?.beginTracking()
        isDragging = true
        return true
    }
    
    public override func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        speedDelegate?.continueTracking()
...
//            .store(in: &cancellables)
    ...
    func generateHapticFeedbackIfNeeded(for value: Double) {
        qualityDelegate?.getQualityValue(value)
        frameDelegate?.getFrameRateValue(value)
        speedDelegate?.getSpeedValue(value)
...
fileprivate extension Slider {
    func cappedXLocation(for touch: UITouch) -> CGFloat {
        ...
        return cappedX
//
//        debugPrint("x : \(x) = slider_w : \(maxValue)")
//        let singlePoint = maxValue / 3
//
//        if (singlePoint/2) <= x {
//            return x
//        }else{
...
fileprivate extension Slider {
    ...
    func makeHapticIndicatorLayer(for value: Double) -> CALayer {
        ...
        layer.backgroundColor = UIColor.gray.cgColor //Constants.indicatorsBackgroundColor
        return layer
    }
    ...
    func makeValueLabel(for value: Double) -> UILabel {
        ...
        let y = bounds.midY + Constants.height+15 / 2
